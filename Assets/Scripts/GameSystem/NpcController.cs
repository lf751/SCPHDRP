...
public class NpcController : MonoBehaviour
{
    ...
    public void ResetNPC(NPC_Data[] npc, NPC_Data[] scp, bool[] simps)
    {
        ...
        for (int v = 0; v < scp.Length; v++)
        {
            ...
            {
                Debug.Log("SCP " + v + " pos " + scp[v].Pos.toVector3() + " Active? " + scp[v].isActive);
    ...
    public int AddNpc(npctype newType, Vector3 where, NPC_Data data = null)
    {
        ...
        if (data == null)
        {
            ...
            Debug.Log("Creating NPC type " + newType + " with ID " + (NPCS.Count));
        }
        else
        {
            newNPC.GetComponent<Map_NPC>().setData(data);
            Debug.Log("Restoring NPC type " + newType + " with ID " + (NPCS.Count));
        }
    ...
    public void NPCManager()
    {
        ...
        if (GameController.ins.currZone == 2 && ZoneMain != Zone3_Main)
        {
            SetMainNPC(Zone3_Main);
        }
        if (GameController.ins.currZone == 1 && ZoneMain != Zone2_Main)
        {
            SetMainNPC(Zone2_Main);
        }
        if (GameController.ins.currZone == 0 && ZoneMain != Zone1_Main)
        {
    ...
    public NPC_Data[] getData()
    {
        ...
        for (int i = 0; i < NPCS.Count; i++)
        {
            ...
            //Debug.Log("Getting NPC data " + i + " de " + NPCS.Count + " tipo " + NPCS[i].data.type);
        }
        return (helper);
    }
    ...
    {
        Debug.Log("Simple npc size = " + Simp_Prefabs.Length);
    ...
    public void GL_Spawn()
    {
        ...
        for (int i = 0; i < spawnSimps.Length; i++)
        {
            ...
            {
                GameObject newSCP = Instantiate(Simp_Prefabs[i], new Vector3(0f, -1000f, 0f), Quaternion.identity, parent.transform);
